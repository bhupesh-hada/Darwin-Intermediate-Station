...
outbound_filename_OutletSKURepo= directorydest+"/IDN_OutletSKURepo_"+datetime.now().strftime("%Y%m%d%H%M%S")
outbound_filename_OutletSKURepo_control= "IDN_OutletSKURepo_"+datetime.now().strftime("%Y%m%d%H%M%S")
...
# DEFINING QUERIES for Outbound Extract - OUTLET_SKU_REPO
outbound_query1 = """call test_OutletSKURepo;""" 

# OPEN OUTBOUND OUTLET_SKU_REPO extract files 
f1 = open(outbound_filename_OutletSKURepo+'.csv', 'w')
f1_ctl = open(outbound_filename_OutletSKURepo+'.ctl', 'w')

# WRITE OUTBOUND OUTLET_SKU_REPO extract files 
writer=csv.writer(f1,delimiter ="|",quoting=csv.QUOTE_NONE,quotechar='')
writer.writerow(['COUNTRY_CODE','DATE','MONTH','OUTLET_CODE','BARCODE'])
...
# Write OUTLET_SKU_REPO control file 
writer=csv.writer(f1_ctl,delimiter ="|",quoting=csv.QUOTE_NONE,quotechar='')
writer.writerow([outbound_filename_OutletSKURepo_control+'.csv'])
