...
outbound_filename_areaNumShoppers= directorydest+"/IDN_AreaNumShoppers_"+datetime.now().strftime("%Y%m%d%H%M%S")
outbound_filename_areaNumShoppers_control= "IDN_AreaNumShoppers"+datetime.now().strftime("%Y%m%d%H%M%S")
...
# DEFINING QUERIES for Outbound Extract - AREA NUM SHOPPERS
outbound_query1 = """call test_AreaNumShoppers;""" 

# OPEN OUTBOUND Area Num Shoppers extract files 
f1 = open(outbound_filename_areaNumShoppers+'.csv', 'w')
f1_ctl = open(outbound_filename_areaNumShoppers+'.ctl', 'w')

# WRITE OUTBOUND Area Num Shoppers extract files 
writer=csv.writer(f1,delimiter ="|",quoting=csv.QUOTE_NONE,quotechar='')
writer.writerow(['COUNTRY_CODE','DATE','MONTH','HOUR_FROM','HOUR_TO','OUTLET_CODE','NUM_TOTAL_SHOPPERS','NUM_SHOPPERS_WITH_UL_ITEMS','TOTAL_SALES_AMT'])
